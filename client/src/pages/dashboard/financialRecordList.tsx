...
const EditableCell: React.FC<EditableCellProps> = ({
  ...
  if (!editable) {
     return <span className="text-slate-300">{value instanceof Date ? value.toLocaleDateString() : value}</span>;
  }
    ...
    () => [
      ...
      columnHelper.accessor('amount', {
        ...
        cell: (props) => {
           ...
           return (
              <div className={clsx("font-medium", type === 'income' ? 'text-emerald-500' : 'text-rose-500')}>
      ...
      columnHelper.accessor('paymentMethod', {
        header: 'Method',
        cell: (props) => <span className="text-xs text-slate-500">{props.getValue()}</span>,
      }),
      columnHelper.accessor('date', {
        header: 'Date',
        cell: (props) => {
           const date = new Date(props.getValue());
           return <span className="text-xs text-slate-400">{date.toLocaleDateString()}</span>;
        }
      }),
