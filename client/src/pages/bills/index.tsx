...
export const Bills: React.FC = () => {
  ...
  const handleMarkUnpaid = async (bill: Bill) => {
      ...
      try {
        const token = await user.getIdToken();
        // Set to previous period or null?
        // If we set to null, it becomes unpaid for all history (if one-time).
        // If recurring, setting to null makes it unpaid for current period.
        // The safest "undo" for current period is to set lastPaidPeriod to null (or previous month string if we tracked history, but we only track last).
        // Since we only track "last_paid_period", clearing it means "never paid" or "not paid recently".
        // This effectively marks it unpaid for today.
        await billService.updateBill(token, bill._id!, {
          lastPaidPeriod: null,
        });
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-10">
      <div className="flex items-center justify-between">
        ...
        <button
           onClick={() => { setIsAdding(true); setEditingBill(null); }}
      ...
      {(isAdding || editingBill) && (
         <div className="mb-6">
            <BillForm
               ...
               onSave={handleSave}
               onCancel={() => { setIsAdding(false); setEditingBill(null); }}
            />
         </div>
      )}
      ...
      <div className="space-y-8">
         ...
         <section>
            <h2 className="text-lg font-semibold text-slate-200 mb-4 flex items-center gap-2">
               ...
               Upcoming
               <span className="text-sm font-normal text-slate-500 ml-2">({upcoming.length})</span>
            </h2>
            {upcoming.length === 0 ? (
               <div className="text-center py-8 bg-slate-900/50 rounded-lg border border-slate-800 border-dashed">
                  <p className="text-slate-500">No upcoming bills for this month.</p>
               </div>
            ) : (
               <div className="grid gap-3">
                  {upcoming.map(bill => (
                     <BillCard
                        ...
                        onEdit={() => { setEditingBill(bill); setIsAdding(false); }}
         ...
         <section>
            <h2 className="text-lg font-semibold text-slate-200 mb-4 flex items-center gap-2">
               ...
               Paid
               <span className="text-sm font-normal text-slate-500 ml-2">({paid.length})</span>
            </h2>
            {paid.length === 0 ? (
               <div className="text-center py-8 bg-slate-900/50 rounded-lg border border-slate-800 border-dashed">
   ...
   return (
      <div className="flex items-center justify-between p-4 bg-slate-900 border border-slate-800 rounded-lg hover:border-slate-700 transition-colors group">
         <div className="flex items-center gap-4">
            ...
            <div>
               <h3 className={`font-medium ${isPaid ? 'text-slate-400 line-through' : 'text-slate-200'}`}>
                  {bill.name}
               </h3>
