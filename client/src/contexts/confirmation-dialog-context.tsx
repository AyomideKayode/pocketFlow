  ...
  return (
    <ConfirmationDialogContext.Provider value={contextValue}>
      ...
      {state.isVisible && state.data && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200" onClick={hideConfirmation}>
          <div
          ...
          >
            <div className="border-b border-slate-800 px-6 py-4">
              <h3 className="text-lg font-semibold text-white">{state.data.title}</h3>
            </div>
            ...
            <div className="flex justify-end gap-3 border-t border-slate-800 bg-slate-900/50 px-6 py-4">
              ...
              <button
                className={cn(
                    ...
                    state.data.variant === 'danger' && "bg-rose-600 hover:bg-rose-500 shadow-rose-900/20",
                    state.data.variant === 'warning' && "bg-amber-600 hover:bg-amber-500 shadow-amber-900/20",
                    (!state.data.variant || state.data.variant === 'info') && "bg-emerald-600 hover:bg-emerald-500 shadow-emerald-900/20"
                )}
